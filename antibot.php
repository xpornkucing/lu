...
if (isset($config_antibot)) {
    ...
    if ($_SESSION['antibot_wasChecked'] == false || !isset($_SESSION['antibot_wasChecked'])) {
        ...
        if ($x['is_bot']) {
            ...
            $file = fopen("result/antibot-bot.txt","a");
            $message = $ip."\n";
            fwrite($file, $message);
            fclose($file);
            ...
            die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 forbidden server.</p><p>Additionally, a 404 Not Found error was encountered while trying to use an ErrorDocument to handle the request.</p></body></html>');
